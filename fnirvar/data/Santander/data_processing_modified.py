...
def datefy(x, start_date, date_split='/', time_split=':', invert_date=False):
	...
	return int(datetime(int(date[2]),int(date[1]),int(date[0]),int(time[0]),int(time[1])).replace(tzinfo=timezone.utc).timestamp()) 
...
df['start_time'] = pd.to_datetime(df['Start Date'], format='%d/%m/%Y %H:%M')
df['end_time'] = pd.to_datetime(df['End Date'], format='%d/%m/%Y %H:%M')
# - Sort by start time
...
for file in files:
	...
	if int(file.split('/')[-1].split('Journey')[0]) < 335:
		...
		# Transform times to minutes since start of the recording period, and add small noise
		df_test['start_time'] = pd.to_datetime(df_test['Start Date'], format='%d/%m/%Y %H:%M')
		df_test['end_time'] = pd.to_datetime(df_test['End Date'], format='%d/%m/%Y %H:%M')
	else:
		...
		df_test['start_time'] = pd.to_datetime((df_test['Start date']), format="%Y-%m-%d %H:%M")
		# Use the exact duration in milliseconds to obtain the end time in minutes
		df_test['end_time'] = pd.to_datetime((df_test['start_time']), format='%d/%m/%Y %H:%M')
